...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://localhost/buzzfeedDB"  # TODO: decide what your new database name will be, and create it in postgresql, before running this new application
...
def index():
    ...
    if form.validate_on_submit():
        if db.session.query(Search).filter_by(term=form.search.data).first():
            ...
            ##################### TASK 5 #####################
            #### Write code for retrieving the search term from the database, and displaying all the articles associated with it.
            #### You can render template all_articles.html to display the article title and the URL as a hyperlink
        else:
...
if __name__ == '__main__':
    db.create_all()
    app.run(debug=True) # NEW: run with this: python challenges.py runserver
